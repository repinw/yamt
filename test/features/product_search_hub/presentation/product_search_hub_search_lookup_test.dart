...
class _FakeOffRepository implements OffProductSearchRepository {
  new({this.searchResults = const <OffProductSearchResult>[]});
...
class _FakeGlobalRepository implements GlobalFoodItemRepository {
  new({this.candidateItems = const <GlobalFoodItem>[]});
...
void main() {
  ...
  test('lookupProductSearchHubProducts merges Firebase and OFF results, '
      'prioritizing Firebase when better', () async {
  ...
  test('lookupProductSearchHubProducts returns both items when they do not '
      'share barcode', () async {
    final now = DateTime.now();
    final globalItem = GlobalFoodItem.create(
      id: 'firebase-item-1',
  ...
  test('lookupProductSearchHubRouteProducts calls lookupProducts callback '
      'if provided', () async {
