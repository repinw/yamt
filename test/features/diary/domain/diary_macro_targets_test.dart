...
void main() {
  group('DiaryMacroTargets.applyCarryover', () {
    const base = DiaryMacroTargets(protein: 160, fat: 80, carbs: 260);

    test('zero carryover leaves targets unchanged', () {
      final result = base.applyCarryover(carryoverKcal: 0, weightKg: 80);
    ...
    test(
      ...
      () {
        ...
        final result = base.applyCarryover(carryoverKcal: 100, weightKg: 80);
    ...
    test('negative carryover reduces fat (25% / 9.3) and carbs (75% / 4.1)'
        ' with protein constant', () {
    ...
    test('Schutzregel B: fat is capped at Fat Floor (0.6 g/kg) and remainder'
        ' redirects to carbs', () {
      ...
      const lowFatBase = DiaryMacroTargets(protein: 160, fat: 50, carbs: 260);
    ...
    test('Carbs floor: carbs never drop below 100g'
        ' (Ketose- & Unterzuckerungsschutz)', () {
      // Base carbs = 120g.
      // Reduction would be ~36.6g -> 83.4g < 100g!
      // Clamped to 100g!
      const lowCarbBase = DiaryMacroTargets(protein: 160, fat: 80, carbs: 120);
