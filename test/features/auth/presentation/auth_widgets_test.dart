...
void main() {
  ...
  testWidgets('LoginForm forgot password button shows placeholder snackbar', (
    tester,
  ) async {
    await tester.pumpWidget(_wrapWithApp(const LoginForm()));

    await tester.tap(find.byKey(const Key('auth_forgot_password_button')));
    await tester.pumpAndSettle();
  ...
  testWidgets('WelcomePage uses desktop layout on wide displays', (
    ...
    expect(find.byType(DesktopAuthLayout), findsOneWidget);
    expect(find.byType(LoginForm), findsOneWidget);
  });
